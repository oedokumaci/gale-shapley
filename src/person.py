...
class Person:
    ...
    def __init__(self, name: str, gender: str, preferences: Tuple = ()) -> None:
        ...
        Args:
            ...
            preferences (tuple, optional): preferences input for person, lower indices are more preferred. Defaults to (). # noqa: B950
    ...
    def __repr__(self) -> str:
        ...
        Returns:
            str: Name and gender of the person. If you want to print preferencess, use print_preferencess method. # noqa: B950
        """
        return f"Name: {self.name}, Gender: {self.gender}"

    def print_preferencess(self) -> None:
    ...
    @property
    def preferences_with_ranks_dict(self) -> dict[int]:
            ...
            if all(isinstance(person, Person) for person in preferences):
                if all(self.gender != person.gender for person in preferences):
                    if len(set(preferences)) == len(preferences):
                        ...
                        raise ValueError(
                            "preferences input should not contain duplicate Person objects"
                        )
                else:
                    raise ValueError(
                        "preferences input can only contain Person objects from the opposite gender"  # noqa: B950
                    )
            else:
                raise TypeError("preferences input should contain Person objects")
        else:
            raise TypeError(
                f"preferences input should be a tuple. Got {type(preferences)} instead"
            )
    ...
    def propose(self, to_propose: Person) -> None:
        ...
        try:
            self.next = list(self.preferences_with_ranks_dict.keys())[
                list(self.preferences_with_ranks_dict.values()).index(
                    next_to_propose_rank
                )
            ]
        except ValueError:
            self.next = self

    def engage(self, proposer: Person) -> None:
    ...
    def get_most_preferred(self, proposals: dict) -> Person:
    ...
    def respond_to_proposals(self) -> None:
        ...
        if len(self.proposals) > 0:
            print(
                f'{self.name} is responding to proposal{"s" if len(self.proposals) > 1 else ""} from {", ".join(list(map(lambda x: x.name,self.proposals.keys())))}'
            )
            ...
            if self.is_single:
                ...
                if (
                    self.preferences_with_ranks_dict[to_accept]
                    < self.preferences_with_ranks_dict[self.match]
                ):
    ...
    def get_opposite_gender(self) -> str:
    ...
    @classmethod
    def print_matches(cls, final: bool = False) -> None:
        """Class method to print all matches in the persons dictionary."""
        if final:
            print(
                f"********** PRINTING {cls.proposing_side.upper()} OPTIMAL MATCH **********"
            )
        else:
    ...
    @classmethod
    def set_proposing_side(cls, side: str) -> None:
